defmodule Satellite.Passes do
  ...
  def list_passes_until(satrec, observer, start_datetime, end_datetime)
  ...
  defp list_passes(satrec, count, observer, start_datetime, passes) do
    new_time = increment_date(start_datetime, 1)
  ...
  defp list_passes_until(_satrec, _observer, start_datetime, end_datetime, passes)
  ...
  defp find_first_pass_for(
    ...
    end_of_pass =
      last_positive_elevation(start_of_pass.datetime, observer, satellite_record, 0.0, 0.0)

    %{start_of_pass: start_of_pass, end_of_pass: end_of_pass}
  end

  defp first_positive_elevation(start_date, observer, satellite_record, elevation, _azimuth)
  ...
  defp decrement_to_lowest_elevation(start_date, observer, satellite_record, elevation, _azimuth)
       when elevation > 0.0 do
    # local_date = :calendar.universal_time_to_local_time(start_date)
    # {{yy, mm, dd},{h, m, s}} = local_date
    # IO.puts "#{yy}-#{mm}-#{dd} #{h}:#{m}:#{s}(local): elevation= #{elevation}"
  ...
  defp last_positive_elevation(start_date, observer, satellite_record, elevation, _azimuth)
       when elevation >= 0.0 do
    # local_date = :calendar.universal_time_to_local_time(start_date)
    # {{yy, mm, dd},{h, m, s}} = local_date
    # IO.puts "#{yy}-#{mm}-#{dd} #{h}:#{m}:#{s}(local): elevation= #{elevation}"
